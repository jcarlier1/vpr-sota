"""
Training script for CosPlace algorithm
"""
...
from algorithms.cosplace.cosplace_model import CosPlaceModel, CosPlaceLoss, get_cosplace_transforms
...
def create_datasets_and_loaders(config: dict):
    ...
    # Get transforms
    train_transform = get_cosplace_transforms(
        image_size=config.get('image_size', 224),
        augment=True
    )
    test_transform = get_cosplace_transforms(
        image_size=config.get('image_size', 224),
        augment=False
    )
...
def train_epoch(model: nn.Module, train_loader: DataLoader, criterion: CosPlaceLoss, 
...
def main():
    parser = argparse.ArgumentParser(description='Train CosPlace for VPR')
    ...
    logger.info("Starting CosPlace training...")
    ...
    logger.info(f"Pooling scales: {config.get('pooling_scales', [1, 2, 4])}")
