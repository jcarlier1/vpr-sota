"""
AP-GeM Training Script for Visual Place Recognition
"""
...
def setup_logging(log_dir):
    ...
    log_file = os.path.join(log_dir, f"apgem_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch, logger):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if isinstance(criterion, TripletLoss):
            ...
            # For contrastive loss, we need to handle the format differently
            features = torch.cat([anchor_features, positive_features, negative_features], dim=0)
            labels = torch.cat([
                torch.zeros(anchor_features.size(0)),  # anchor
                torch.ones(positive_features.size(0)),  # positive (same as anchor)
                torch.zeros(negative_features.size(0))  # negative
            ]).to(device)
...
def save_checkpoint(model, optimizer, epoch, loss, results, checkpoint_path):
    ...
    checkpoint = {
        ...
        'model_config': {
            ...
            'regional_pooling': model.regional_pooling,
            'local_whitening': model.local_whitening
        }
    }
...
def train_apgem(config):
    ...
    # Create model
    logger.info("Creating AP-GeM model...")
    model = create_apgem_model(
        architecture=config['architecture'],
        pretrained=config['pretrained'],
        gem_p=config['gem_p'],
        whitening=config['whitening'],
        whitening_dim=config.get('whitening_dim'),
        regional_pooling=config.get('regional_pooling', False),
        local_whitening=config.get('local_whitening', False)
    )
    model.to(device)
    
    logger.info(f"Model output dimension: {model.output_dim}")
...
def main():
    parser = argparse.ArgumentParser(description='Train AP-GeM for Visual Place Recognition')
